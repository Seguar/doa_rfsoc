...
class doaMtsOverlay(Overlay):
    ...
    def __init__(self, bitfile_name='mts.bit', **kwargs):
        ...
        self.signal = np.zeros_like(self.dac0_player.shape[0])
    ...
    def configure_adc(self, dev_num, nyquist_zone=NYQUIST_ZONE, centre_freq=CENTRE_FREQ, event_src=EVENT_SRC, mixer_phase=0.0, gain=A_GAIN):
            """
    ...
    def configure_adcs(self):
            ...
            for n in range(MAX_ADC_TILES):
                if (bitvector & 0x1):
                    for i in range(self.PATH_PER_TILE):
                        self.configure_adc(n*self.PATH_PER_TILE+i, self.nyquist_zone[n], self.centre_freq[n], self.event_src, self.phases[n*self.PATH_PER_TILE+i], self.a_gain[n*self.PATH_PER_TILE+i])
                bitvector = bitvector >> 1
    ...
    def configure_dac(self, dev_num, nyquist_zone=NYQUIST_ZONE, centre_freq=CENTRE_FREQ, event_src=EVENT_SRC, mixer_phase=0.0, gain=D_GAIN):
        """
    ...
    def configure_dacs(self):
        ...
        for n in range(MAX_ADC_TILES):
            if (bitvector & 0x1):
                for i in range(self.PATH_PER_TILE):
                    self.configure_dac(n*self.PATH_PER_TILE+i, self.d_nyquist_zone[n], self.d_centre_freq[n], self.event_src, self.d_phases[n*self.PATH_PER_TILE+i], self.d_gain[n*self.PATH_PER_TILE+i])
            bitvector = bitvector >> 1
    ...
    def handle_commands(self, commandList):
        for command_str in commandList:
            ...
            match command:
                ...
                case "phase":
                    # self.phases = var
                    self.phases[0] = var[0]/100
                    self.phases[2] = var[1]/100
                    self.phases[4] = var[2]/100
                    self.phases[6] = var[3]/100
                    self.configure_adcs()
                    # self.sync_tiles()
                case "dphase":
                    # self.d_phases = var
    ...
    def calibrate_xm655(self):
            ...
            # coefs_new = np.clip(coefs_new, -179, 179)
            self.phases[0] = coefs_new[0]
            self.phases[2] = coefs_new[1]
            self.phases[4] = coefs_new[2]
            self.phases[6] = coefs_new[3]            
            self.configure_adcs()
            # self.sync_tiles()
    ...
    def filtSig(self, sig, fs, bw):
        ...
        sigFd = np.fft.fftshift(np.fft.fft(sig))

        # Define limits for the new frequency range
        sigNewLim = np.arange(round((fs/2 - bw/2) / fStep), round((fs/2 + bw/2) / fStep))

        # Initialize a new zero array
        sigNew = np.zeros_like(sigFd)

        # Copy the filtered frequency components
        sigNew[sigNewLim] = sigFd[sigNewLim]

        # Perform inverse FFT and shift
        sigFilt = np.fft.ifft(np.fft.ifftshift(sigNew))
